...
STUDY_NAME = 'som_qe_te_sil_v1' # Nome do estudo que você quer carregar
STORAGE_DB = 'sqlite:///../data/som_optimization.db'
FINAL_MODEL_PATH = '../models/final_som_model_3_obj.pth'
NUM_EPOCHS_FINAL = 20
...
if __name__ == '__main__':
    # --- Passo 1: Carregar estudo, filtrar e selecionar o melhor trial ---
    ...
    df_successful = df_all_trials.dropna(subset=['values_0', 'values_1', 'values_2'])
    VALID_SOM_X_MIN = 25; VALID_SOM_X_MAX = 42
    ...
    # a) Isolar as colunas de objetivos
    objectives = df_consistent[['values_0', 'values_1', 'values_2']].values
    # b) Normalizar os valores para a escala [0, 1]
    scaler = MinMaxScaler(); normalized_objectives = scaler.fit_transform(objectives)
    # c) Ajustar objetivos de maximização (Silhouette é o índice 2)
    normalized_objectives[:, 2] = 1 - normalized_objectives[:, 2]
    # d) Calcular a distância de cada trial ao ponto ideal (0, 0, 0)
    distances_to_ideal = np.linalg.norm(normalized_objectives, axis=1)
    # e) Encontrar o trial com a menor distância
    best_trial_index = np.argmin(distances_to_ideal)
    best_trial_row = df_consistent.iloc[best_trial_index]
    
    print("\n--- Melhor Configuração Selecionada (Equilíbrio Multivariado) ---")
    print(f"Número do Trial: {int(best_trial_row['number'])}")
    print(f"Valores Originais (QE, TE, Sil): [{best_trial_row['values_0']:.4f}, {best_trial_row['values_1']:.4f}, {best_trial_row['values_2']:.4f}]")
    best_params = {
    ...
    # --- Passo 2: Preparar os DataLoaders ---
    ...
    # --- Passo 3: Construir e treinar o modelo final ---
    ...
    # --- Passo 4: Salvar o modelo treinado ---
    FINAL_MODEL_PATH = f"../models/final_som_model_{best_params['som_x']}x{best_params['som_x']}.pth"
    ...
    output_graph_path = f'training_history_{best_params["som_x"]}x{best_params["som_x"]}.png'
