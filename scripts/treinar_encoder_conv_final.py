...
class DocumentTokenDataset(Dataset):
    """Dataset que carrega um documento (uma sequência de tokens) por vez."""
...
class AttentionEncoder(nn.Module):
    """Encoder: Refina uma sequência de tokens usando um Transformer Encoder."""
    def __init__(self, input_dim, n_layers=2, num_heads=4):
        super().__init__()
        encoder_layer = nn.TransformerEncoderLayer(d_model=input_dim, nhead=num_heads, batch_first=True, dropout=0.1)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)

    def forward(self, x):
        x = x.unsqueeze(0)
        refined_x = self.transformer_encoder(x)
        return refined_x.squeeze(0)

class Decoder(nn.Module):
    """Decoder: Tenta recriar a sequência original a partir da sequência refinada."""
...
class AttentionAutoencoder(nn.Module):
    """Junta o Encoder e o Decoder em um único modelo."""
    def __init__(self, input_dim=1536, encoder_layers=2, num_heads=4, decoder_layers=2):
        super().__init__()
        self.encoder = AttentionEncoder(input_dim, encoder_layers, num_heads)
        self.decoder = Decoder(input_dim, decoder_layers)

    def forward(self, x):
        refined_sequence = self.encoder(x)
        reconstructed_x = self.decoder(refined_sequence)
...
STUDY_NAME = 'attention_autoencoder_conv_v2' # Nome do estudo que você quer carregar
STORAGE_DB = 'sqlite:///../data/autoencoder_optimization.db'
FINAL_ENCODER_PATH = '../models/final_attention_conv_encoder.c  pth' # Salvaremos apenas o encoder
...
if __name__ == '__main__':
    ...
    print("\n--- Melhor Configuração Encontrada (menor erro de reconstrução) ---")
    print(f"Número do Trial: {best_trial.number}")
    print(f"  - Valor (Erro de Reconstrução): {best_trial.value:.6f}")
    ...
    # Criar DataLoaders para cada fase, incluindo validação para cada uma
    ...
    plt.axvline(x=PHASE_1_EPOCHS - 0.5, color='red', linestyle='--', label='Início da Fase de Especialização')
